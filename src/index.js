...
app.post('/api/v1/user', authenticationRequired, (request, response) => {
    withErrorHandler(async () => {
        let id = await models.user.create(request.userinfo.sub);
...
// Get list of IDs
// GET /api/v1/user
app.get('/api/v1/user', authenticationRequired, (request, response) => {
    withErrorHandler(async () => {
        let ids = await models.user.getBySub(request.userinfo.sub);
